...
proc duv_compile { } {
  global Path_DUV
  vcom -explicit $Path_DUV/alu.vhd
}

proc tb_compile {} {
  global Path_TB
  vlog -mixedsvvh $Path_TB/alu_assertions.sv  $Path_TB/alu_tb.sv
}

#------------------------------------------------------------------------------
proc sim_start {SIZE ERRNO} {
  puts "\nStart simulation :"
  vsim -t 1ns -GSIZE=$SIZE -GERRNO=$ERRNO work.alu_tb
  #add wave -r *
  #wave refresh
  run -all
}
...
#------------------------------------------------------------------------------
proc do_all {SIZE ERRNO} {
  duv_compile
  tb_compile
  sim_start $SIZE $ERRNO
}
