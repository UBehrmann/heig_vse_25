...
proc vhdl_compile { } {
  ...
  vcom -2008 -mixedsvvh $Path_VHDL/ble_packet_analyzer.vhd
  vlog -sv -mixedsvvh $Path_TB/ble_packet_analyzer_tb.sv
}

#------------------------------------------------------------------------------
proc sim_start { testcase errno} {

  vsim -t 1ns -GTESTCASE=$testcase -GERRNO=$errno work.packet_analyzer_tb
  add wave -r *
  wave refresh
  run -all
}
